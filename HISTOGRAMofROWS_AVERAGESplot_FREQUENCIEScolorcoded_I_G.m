...
function    HISTOGRAMofROWS_AVERAGESplot_FREQUENCIEScolorcoded_I_G                                                                                                                                                                                       (...
            input, filename                                                                                                                                                                                                                                   )   
%%  Description                                                                                                                                                                                            
...
%----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------% 
%    Average, sample variance, and the measure of the range of each row                                                                                                                        %  
%----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------%

averages=mean(input,2);
    
sample_variance=var(input')';

max_measure_of_range=max(max(input')'-min(input')');


%----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------% 
%    Each row's unique entries and their relative frequencies                                                                                                                                  %  
%----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------%

histogram_plot_data=RELATIVEfrequencies_row_M_M(input);
...
%       -Assign the row of occurence to the corresponding entry of the vector <rows>                                                                                                           %
%       -Assign the relative frequency to the corresponding entry of the vector ,<relative_frequencies>                                                                                        %
%----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------%


values=[];rows=[];relative_frequencies=[];

for i=1:length(histogram_plot_data(:,1,1))
    
    for j=1:length(histogram_plot_data(1,:,1))
        
        if histogram_plot_data(i,j,2)~=0
            values(end+1)=histogram_plot_data(i,j,1);
            rows(end+1)=i;
            relative_frequencies(end+1)=histogram_plot_data(i,j,2);
        end
...
axes_histogram = axes('Parent',graph,'FontName','Palatino Linotype','FontSize',12,'FontSize',12,'Position',...
                      [0.0483267479549314 0.10387323943662 0.794162613747196 0.805281690140842],...
                      'CLim',[1/number_of_columns 10/number_of_columns]);
...
scatter(rows,values,point_size,relative_frequencies,'Marker','.');
...
axes_histogram_x_label=xlabel('Position','FontWeight','bold','FontSize',16,'FontName','Palatino Linotype');
...
%%  Hold the figure then plot the average intensities                                                                                                                                                      
...
plot(averages,'Color',[0.2 0.84 .22],'LineWidth', 2);
...
cbar=colorbar('peer',axes_histogram,'position',[0.915690027631546 .12 .015 .4]);
...
annotation(graph,'textbox',...
    ...
    ['Range_{max}:   ' sprintf('%.5f',max_measure_of_range)],'',...
    ['N:   ' int2str(number_of_columns)]},...
    'FontSize',12,...
